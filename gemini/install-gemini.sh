...
if ! command -v node >/dev/null 2>&1; then
  echo "Forge compatibility entrypoint: gemini/install-gemini.sh" >&2
  echo "Node.js 18+ is required. Install Node.js and rerun, or use the desktop installer." >&2
  exit 1
fi

echo "Forge compatibility entrypoint: gemini/install-gemini.sh"
echo "Delegating to: forge install gemini"
...
[ -n "${FORGE_GEMINI_EXA_KEY:-}" ] && ARGS+=(--exa-api-key "$FORGE_GEMINI_EXA_KEY")
