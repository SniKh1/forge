...
This is a **universal self-improvement system** that learns from ALL skill experiences, not just PRDs. In Forge, it should enrich the shared memory model rather than build a separate source of truth. It implements a feedback loop with:
...
## Forge Memory Alignment

Canonical Forge memory targets:
...
The semantic / episodic / working-memory model below is still useful for reasoning about learning, but durable writes should resolve into the Forge targets above.
