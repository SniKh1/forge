...
def main():
    config_path = Path(os.environ.get("CLAUDE_MCP_FILE", os.path.expanduser("~/.claude/.mcp.json")))
    data = json.loads(config_path.read_text(encoding="utf-8"))
    servers = data.get("mcpServers", {})

    for name in ["sequential-thinking", "context7", "memory", "fetch", "playwright", "deepwiki", "exa"]:
