...
全部 115 个 Skills 随安装脚本自动安装到 `~/.claude/skills/`，无需单独下载或选择模块。

安装完成后，Claude Code 会根据关键词自动匹配并调用对应的 Skill。你也可以通过 `/skill-name` 手动调用。

### 2.1 Skill 分类速查

| 分类 | 示例 |
|------|------|
| 核心工作流 | superpowers、tdd-workflow、coding-standards、verification-loop |
| 前端 | frontend-design、frontend-patterns、theme-factory、canvas-design |
| 后端 | backend-patterns、postgres-patterns、api-design |
| Java | java-coding-standards、springboot-patterns/security/tdd |
| Python | python-patterns、django-patterns/security/tdd |
| 安全 | security-review、security-scan |
| 文档 | doc-coauthoring、docx、pdf、pptx、xlsx |
| 测试 | webapp-testing、e2e-testing、tdd-workflow |
...
| 联网搜索 | `mcp__exa__web_search_exa` | "2026 年最新的 Next.js 特性" |
