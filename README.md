...
| 桌面端运行环境排查 | `bash scripts/diagnose-forge-runtime.sh` / `powershell -ExecutionPolicy Bypass -File .\scripts\diagnose-forge-runtime.ps1` |
...
- 下载安装包后，终端用户不需要 Rust / Cargo 之类的构建环境
- Forge Desktop 当前仍依赖本机 `Node.js 18+` 来执行内置 Forge CLI
- 某些 MCP 配置流程会额外使用 `Python`
- 个别安装脚本目前仍会检查 `git`
...
如果桌面端安装后仍显示“无法读取 Forge 状态”，请直接运行：
...
诊断说明见 [docs/user/desktop-runtime-troubleshooting.md](docs/user/desktop-runtime-troubleshooting.md)。
