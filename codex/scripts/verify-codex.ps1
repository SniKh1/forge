...
if ((Test-Path (Join-Path $ForgeHome "rules\security.md")) -and (Test-Path (Join-Path $ForgeHome "agents\planner.md")) -and (Test-Path (Join-Path $ForgeHome "stacks\frontend.md"))) {
  Pass "key playbooks installed"
} else {
  Fail "key playbooks missing"
}
